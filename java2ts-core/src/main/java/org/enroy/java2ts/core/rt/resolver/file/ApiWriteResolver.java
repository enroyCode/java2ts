package org.enroy.java2ts.core.rt.resolver.file;

import org.apache.commons.lang3.StringUtils;
import org.enroy.java2ts.core.entity.ApiEntity;
import org.enroy.java2ts.core.entity.ApiMethodEntity;
import org.enroy.java2ts.core.entity.TypeParameter;
import org.enroy.java2ts.core.rt.TypeProcessPool;
...
import static org.enroy.java2ts.core.commons.Consts.*;
...
//          })
    ...
    private void processMethodParamNotes(ApiMethodEntity method, Map<String, String> paramNotes, StringBuilder methodsSb) {
        if (CollectionUtils.isEmpty(paramNotes)) {
            ...
            for (Map.Entry<String, String> noteEntry : paramNotes.entrySet()) {
                notesSb.append(ENTER).append(TAB).append(" * ").append(String.format("@param %s %s", noteEntry.getKey(), noteEntry.getValue()));
            }
    ...
    private Map<String, String> processApiMethod(ApiMethodEntity method, String apiPath, StringBuilder methodsSb, TypeProcessPool importsClassPool) {
        ...
        if (StringUtils.isNotBlank(config.getPromiseResolveReturn())) {
            clientSb.append(PERIOD).append(config.getPromiseResolveReturn());//
        }
